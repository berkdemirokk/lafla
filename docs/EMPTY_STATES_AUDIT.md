...
interests selected, etc. — lands on a screen that looks broken,
...
| 9 | `freechat.tsx` | Local conversation engine unavailable | Resolved | Deterministic engine has no provider dependency |
...
## 9. FreeChat — provider dependency removed

**Files:** `apps/mobile/app/freechat.tsx`,
`apps/mobile/lib/local-conversation-engine.ts`, and
`apps/mobile/lib/local-freechat-store.ts`

**Resolution.** Free Chat no longer has a provider configuration empty state.
It classifies intent and entities, advances explicit turn state, and chooses a
non-repeating response locally. The daily free quota uses a serialized local
counter and is consumed only after input safety succeeds. The same path works
online, offline, and in contributor builds without secrets.
...
1. **Achievements when 0 earned.** Punishing grid of locked padlocks with a
   flat zero-width progress bar and zero motivation copy. New user opens, sees
   only 🔒🔒🔒, closes.

2. **Streak calendar with 0 days of activity.** Giant `0 🔥` as the first
   thing on the screen. The hint at the bottom is good but the user already
   bounced.

3. **Profile with no completed lessons.** The screen has little evidence of
   what progress will look like until the first lesson is completed.
