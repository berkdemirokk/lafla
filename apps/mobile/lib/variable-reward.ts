...
async function readState(): Promise<{ counter: number; threshold: number }> {
  try {
    ...
    const counter = cRaw ? parseInt(cRaw, 10) : 0;
    let threshold = tRaw ? parseInt(tRaw, 10) : NaN;
    if (Number.isNaN(threshold)) {
      threshold = pickThreshold();
      await AsyncStorage.setItem(K_THRESHOLD, String(threshold)).catch(() => {});
    }
    return {
    ...
    return { counter: 0, threshold: pickThreshold() };
  }
}
...
export async function bumpSurpriseCounter(): Promise<boolean> {
  ...
  const pending = await getPendingSurpriseLessonId();
  if (pending) return true; // already triggered, waiting consumption

  const next = counter + 1;
  await AsyncStorage.setItem(K_COUNTER, String(next)).catch(() => {});
...
export async function getPendingSurpriseLessonId(): Promise<string | null> {
  try {
    return await AsyncStorage.getItem(K_PENDING_ID);
  } catch {
    return null;
  }
}
...
export async function ensureSurpriseSceneIfPending(opts: {
  ...
  await AsyncStorage.setItem(K_PENDING_ID, pick.lessonId).catch(() => {});
  await AsyncStorage.setItem(K_LAST_SHOWN, new Date().toISOString()).catch(
    () => {},
  );
...
export async function consumeSurprise(): Promise<void> {
  try {
    await Promise.all([
      AsyncStorage.removeItem(K_PENDING_ID),
      AsyncStorage.setItem(K_COUNTER, "0"),
      AsyncStorage.setItem(K_THRESHOLD, String(pickThreshold())),
    ]);
  } catch {
    // best effort
  }
